...
import {OPENAI_KEY, STORAGE_DIR, TEMP_DIR} from '../keys';
...
export class EpubUtilService {
  ...
  walk(dir: string, files: Array<FileType> = []): Array<FileType> {
    const dirFiles = fs.readdirSync(dir)
    for (const f of dirFiles) {
      const stat = fs.lstatSync(dir + '/' + f)
      if (stat.isDirectory()) {
        this.walk(dir + '/' + f, files)
      } else {
  ...
  async epubSplitter(epub: string, ceeMediaRepository: CeeMediaRepository, parentCeeMedia: CeeMedia, isbn: string): Promise<boolean> {
    ...
    for (const idref of idrefArray) {
      ...
      const allNavPoints: NavPoint[] = this.getAllNavPoints($toc, xhtmlFileNavMapTag);
  ...
  getAllNavPoints($: cheerio.CheerioAPI, element: any): NavPoint[] {
    const navPoints: NavPoint[] = [];
    const thisClass = this;

    $(element).find('navPoint').each(function (index: any, element: any) {
      const navPoint = $(element);
      ...
      thisClass.getAllNavPoints($, navPoint);
  ...
  async createEpubCeeMedia(epubPath: string, ceeMediaRepository: CeeMediaRepository, resource: string, isbn: string, collection: string): Promise<CeeMedia> {
    const cheerio = require('cheerio');
  ...
  getTocDirectory(pathName: string): string {
  ...
  getThumbnailPath(tocFolderPath: string): string {
  ...
  async generateEpubDescription(epubPath: string, model: string, maxContextChars: number): Promise<string> {
    ...
    const tocDir = `${tempBookPath}/${this.getTocDirectory(tempBookPath)}`;
    const title = this.getTitle(tocDir);
    const toc = this.getTableOfContents(tocDir);
    // Loop through list of files and get text until we reach context limit
    //const filenames = fs.readdirSync(tocDir);
    const filenames = this.walk(tocDir);
    let content = '';
    ...
    for (const filename of filenames) {
      ...
      const fileContent = this.getFirstPages(filename.path);
  ...
  getTableOfContents(filePath: string): string[] {
    const cheerio = require('cheerio');
  ...
  getFirstPages(filePath: string): string {
    const fileData = fs.readFileSync(filePath, 'utf-8');
    return this.removeHtmlTags(fileData);
  };

  removeHtmlTags(xhtmlString: string) {
    const cheerio = require('cheerio');
